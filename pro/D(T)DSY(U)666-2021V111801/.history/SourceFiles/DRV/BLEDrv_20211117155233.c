...
{ 
	...
	if( _SUCCESS == BleDrv_BleComm( COMM_BLE_EADV, BLE_RW_WRITE, V_ucTempBuff ) )
	{
		...
		GStr_Monitor.ucBleAdvCnt = 0;					/* 蓝牙广播计时器清零 */
	}
	return ( V_ucResult );
}
...
void BleDrv_BleStatuChk(void)
{
	...
	do{
		...
		{
			GStr_Monitor.ucBleConnectCnt = 0;			/* 非连接状态下清零，确保连接时从0开始计时*/
			BleDrv_SetBleRunPara();						/*设置运行参数，起动广播*/
		}
		else if( BLE_STAT_ADV_ON == V_ucBleStatus )		/* 处于广播状态*/
		{
			GStr_Monitor.ucBleConnectCnt = 0;			/* 非连接状态下清零，确保连接时从0开始计时*/
			/*间隔三个小时，定时广播一下*/
		...
		{
			if( ( (GStr_Flag.ucMeterMode & F_FACTORY_MODE) != F_FACTORY_MODE )		/* 厂内模式的表不会去判心跳 */
				&& ( (++GStr_Monitor.ucBleConnectCnt) > 3) )	/* 连接状态下连续2分钟没有收到心跳信号（最多3分钟），直接复位*/
			{
				GStr_Monitor.ucBleConnectCnt = 0;
				GStr_Flag.ucBLESTATFlag = BLE_STAT_RST;	/*蓝牙模块需要重启，重新设置参数*/
			}
		}
