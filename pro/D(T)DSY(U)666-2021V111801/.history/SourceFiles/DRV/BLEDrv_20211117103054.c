...
void BleDrv_BleStatuChk(void)
{
	...
	do{
		...
		{ 
			...
			{
				GStr_Monitor.ucBleRestCnt=0;
				BleDrv_SetBleStat();			/*设置为广播状态*/
			}
			/*间隔三个小时，定时广播一下*/
			if( GStr_Monitor.ucBleRestCnt > 180 )
			{
				GStr_Monitor.ucBleRestCnt = 0;
				BleDrv_SetBleStat();			/*设置为广播状态*/
			}
		...
		else if( BLE_STAT_IDLE == V_ucBleStatus )	/* 处于空闲状态*/
		{
			GStr_Monitor.ucBleRestCnt=0;
			BleDrv_SetBleRunPara();				/*设置运行参数，起动广播*/
		}
		...
		if( (GStr_Monitor.ucBleRestCnt & B0000_0011) != B0000_0011 )
		{ 
			break;
		}
