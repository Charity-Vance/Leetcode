			...
			case API_COMM_STEP6:
			{
				...
				else
				{
					...
					if( (Gstr_UartComm[v_ucChID].ucaCommRXBuff[16] == 0x82)&&!(LibPub_0xFFCheckInt(&Gstr_UartComm[v_ucChID].ucaCommRXBuff[API_DLT645_MACADDRLSB], 6)) )
					{
						...
						{
							GStr_Monitor.ucBleConnectCnt = 0;				/* 上报直接将心跳计数清零，避免快速断开连接导致判断不及时*/
						...
						{
							GStr_Flag.ucBLESTATFlag = BLE_STAT_ERRREP;		/* 模块连接上报异常 */
						}
					}
				}
			}break;
...
void ApiDLT645_FrameAnalysis( uchar8 v_ucChID)
{
	...
	GStr_Monitor.ucBleConnectCnt = 0;	/* 符合645帧格式就将连接计时器清零，证明蓝牙心跳正常 */	
	...
	if ( Gstr_UartComm[v_ucChID].ucCommChStatus != API_COMM_STATUS_ERROR )
	{
		...
		if ( CH_UART_UART1 == v_ucChID)	// 目前是蓝牙
		{
			...
			GStr_Monitor.ucBleAdvCnt = 0;	//蓝牙定时校验计数清零，有蓝牙通信就清零
		}
...
static uchar8 ApiDLT645_ReadData07( uchar8 v_ucChID )
{
	...
	/* 蓝牙的心跳包 */
	if( (0xEEEE0100 == V_ulDI3DI2DI1DI0)		/* 自扩命令 */
		&& (0x04 == Gstr_UartComm[v_ucChID].ucaCommTXBuff[API_DLT645_LEN] ))	
	{
		GStr_Monitor.ucBleConnectCnt = 0;	/* 连接计时器清零 */
